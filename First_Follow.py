...
class First_Follow:
    ...
    def _first_de_cadena(self, cadena):
        """
        Calcula el conjunto FIRST para una secuencia de símbolos (una producción).

        Reglas aplicadas:
        1. Si el primer símbolo es un terminal, es el único miembro del conjunto FIRST.
        2. Si es un no terminal, su conjunto FIRST se añade al resultado.
        ...
        Returns:
            set: El conjunto FIRST de la cadena.
        """
        ...
        for simbolo in cadena:
            if simbolo in self.gramatica.terminales:
                ...
                first_simbolo = self.first.get(simbolo, set())
                resultado.update(first_simbolo - {'e'})
                
                # Si el no terminal no deriva en epsilon, no se puede seguir.
                if 'e' not in first_simbolo:
                    return resultado
                ...
                for produccion in self.gramatica.obtener_producciones(nt):
                    for i, simbolo in enumerate(produccion):
                        if simbolo in self.gramatica.no_terminales:
                            ...
                            # Aplica la Regla 2.
                            first_beta = self._first_de_cadena(beta)
                            self.follow[simbolo].update(first_beta - {'e'})
